...
const PortfolioManager: React.FC = () => {
  const { environment, isLive, isSimulation: _isSimulation } = useTradingEnv();
  ...
  const getSectorAllocation = () => {
    ...
    assets.forEach(asset => {
      const currentAllocation = sectorMap.get(asset.sector) || 0;
      sectorMap.set(asset.sector, currentAllocation + asset.allocation);
    });
  ...
  const getRiskLevel = (pnlPercentage: number) => {
    if (pnlPercentage < -5) return { level: 'high', color: 'text-red-500', bg: 'bg-red-500/20' };
    if (pnlPercentage < 0) return { level: 'medium', color: 'text-yellow-500', bg: 'bg-yellow-500/20' };
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-6">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-400 mt-2">
            Monitor and manage your trading portfolio across all accounts
          </p>
        ...
        <div className="bg-gray-800 rounded-lg p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className={`text-sm ${
                metrics.totalPnLPercentage >= 0 ? 'text-green-400' : 'text-red-400'
              }`}>
        ...
        <div className="bg-gray-800 rounded-lg p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className={`text-sm ${
                metrics.dayChangePercentage >= 0 ? 'text-green-400' : 'text-red-400'
              }`}>
                {metrics.dayChangePercentage >= 0 ? '+' : ''}{metrics.dayChangePercentage.toFixed(2)}%
              </p>
            </div>
      ...
      {/* Time Frame and Account Type Filters */}
      <div className="flex items-center space-x-6 mb-6">
        <div className="flex items-center space-x-2">
          ...
          <div className="flex space-x-1">
            {['1D', '1W', '1M', '3M', '1Y'].map((period) => (
              <button
                key={period}
                onClick={() => setSelectedTimeframe(period as '1D'|'1W'|'1M'|'3M'|'1Y')}
        ...
        <div className="flex items-center space-x-2">
          ...
          <select
            ...
            onChange={(e) => setSelectedAccountType(e.target.value as 'all' | 'prop-firm' | 'retirement' | 'tax-free' | 'taxable')}
          ...
          >
            ...
            <option value="prop-firm">Prop Trading</option>
            <option value="retirement">Retirement (RRSP, LIRA, etc.)</option>
            <option value="tax-free">Tax-Free (TFSA, FHSA)</option>
            <option value="taxable">Taxable Accounts</option>
          </select>
        </div>
      </div>
            ...
            <table className="w-full">
              <thead className="bg-gray-700">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase">Market Value</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase">P&L</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase">Allocation</th>
                </tr>
              </thead>
                ...
                {assets.map((asset, index) => (
                  <tr key={index} className="hover:bg-gray-700/50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <div className={asset.pnl >= 0 ? 'text-green-400' : 'text-red-400'}>
                        {asset.pnl >= 0 ? '+' : ''}${asset.pnl.toFixed(2)}
                      </div>
                      <div className={`text-xs ${asset.pnlPercentage >= 0 ? 'text-green-400' : 'text-red-400'}`}>
          ...
          <div className="bg-gray-800 rounded-lg p-6">
            ...
            <div className="space-y-4">
              {filteredAccounts.map((account) => (
                <div key={account.id} className="border border-gray-700 rounded-lg p-4">
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      <h4 className="text-sm font-medium text-white">{account.name}</h4>
                      <div className="flex items-center space-x-2">
                        <span className="text-xs text-gray-400">{ACCOUNT_TYPES[account.type].displayName}</span>
                        {ACCOUNT_TYPES[account.type].taxAdvantaged && (
                          <span className="text-xs px-1 py-0.5 bg-green-500/20 text-green-400 rounded">Tax Advantaged</span>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <div>
                      ...
                      <div className="text-white">${account.balance.toLocaleString()}</div>
                    </div>
                    <div>
                      <span className="text-gray-400">Equity:</span>
                      <div className="text-white">${account.equity.toLocaleString()}</div>
                    </div>
                    <div className="col-span-2">
                      <span className="text-gray-400">P&L:</span>
                      <div className={account.pnl >= 0 ? 'text-green-400' : 'text-red-400'}>
                        {account.pnl >= 0 ? '+' : ''}${account.pnl.toFixed(2)} ({account.pnlPercentage >= 0 ? '+' : ''}{account.pnlPercentage.toFixed(2)}%)
                      </div>
                    </div>
                  </div>
                </div>
