...
const SectionSidebar: React.FC<{ isDevelopment: boolean }> = ({ isDevelopment }) => {
  ...
  const current = useMemo(() => {
    ...
    // find section whose path is a prefix or whose subSection matches
    ...
    if (!match) {
      match = candidates.find(s => (s.subSections || []).some(sub => sub.path === path || path.startsWith(sub.path + '/')));
    }
    if (!match) return undefined;
    // Filter subsections by env/role similar to top nav
    ...
    <aside className="sticky top-20 space-y-3">
      <div className="bg-gray-900/70 border border-gray-800 rounded-lg p-4">
        ...
        <nav className="space-y-1">
          {/* Optional overview link to the section root */}
          <Link
            to={current.path}
