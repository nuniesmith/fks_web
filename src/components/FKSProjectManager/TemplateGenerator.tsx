...
const TemplateGenerator: React.FC<Props> = ({ onLog, disabled }) => {
  const [open, setOpen] = useState(false);
  const [fileName, setFileName] = useState('MyNewStrategy');
  const [type, setType] = useState(TEMPLATE_TYPES[1].id);
  const [loading, setLoading] = useState(false);

  const submit = async () => {
    if (!fileName) return;
    setLoading(true);
    onLog({ timestamp: new Date().toLocaleTimeString(), message: `Generating ${type} template…`, type: 'info' });
    try {
  ...
  if (!open) {
    return (
      <button disabled={disabled} onClick={() => setOpen(true)} className="w-full btn-secondary flex items-center justify-center gap-2 disabled:opacity-50 text-sm">
  ...
  return (
    <div className="mt-2 space-y-3">
      <div className="flex items-center justify-between">
        <h4 className="font-semibold text-sm">Generate Template</h4>
      ...
      <button onClick={submit} disabled={loading || disabled} className="btn-primary w-full text-sm disabled:opacity-50">
