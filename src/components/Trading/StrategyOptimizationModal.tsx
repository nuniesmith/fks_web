...
interface Props {
  ...
  onOptimized: (params: Record<string, any>) => void;
}
...
const StrategyOptimizationModal: React.FC<Props> = ({ open, onClose, code, language, parameters, onOptimized }) => {
  ...
  const run = async () => {
    ...
    try {
      const r = await optimizeStrategy({ code, language, parameters });
      setResult(r.parameters);
    } catch (e: any) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-gray-800 rounded-xl p-6 max-w-2xl w-full mx-4">
        ...
        <div className="space-y-3 text-sm">
          <div className="text-gray-300">Find better parameters automatically (Optuna-ready mock).</div>
          <button disabled={loading} onClick={run} className="px-4 py-2 bg-purple-600 hover:bg-purple-700 rounded text-white disabled:bg-gray-600">
          ...
          {result && (
            <div className="bg-gray-900 rounded-lg p-3">
              ...
              <pre className="text-xs text-gray-200 overflow-x-auto">{JSON.stringify(result, null, 2)}</pre>
