...
const AuthenticationSelector: React.FC<AuthenticationSelectorProps> = ({
  ...
  const [selectedProvider, setSelectedProvider] = useState<'authelia' | 'google' | null>(null);
  ...
  const handleAuthentikLogin = async (preferPasskey: boolean = true) => {
    try {
      setSelectedProvider('authelia');
      const result = await login(preferPasskey, 'authelia');
      ...
      const errorMessage = err instanceof Error ? err.message : 'Authentik authentication failed';
      if (onError) {
    ...
    <div className={`bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 ${className}`}>
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          ...
          <button
            onClick={() => handleAuthentikLogin(true)}
            disabled={loading}
          ...
          >
            {loading && selectedProvider === 'authelia' ? (
              <Loader className="animate-spin h-5 w-5 mr-2" />
            ) : (
              <Shield className="h-5 w-5 mr-2" />
            )}
            Sign in with Authentik {showPasskeyOption && '(Passkey)'}
          </button>

          {showPasskeyOption && (
            <button
              onClick={() => handleAuthentikLogin(false)}
              disabled={loading}
