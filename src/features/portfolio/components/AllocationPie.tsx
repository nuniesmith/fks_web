...
const LazyPieBundle = React.lazy(() => import('../../lazy/RechartsPie')); // dynamic pie sub-bundle
	...
	const chartData = entries.map(([name, value]) => ({ name, value }));
		return (
			<div className="p-4 bg-white/5 rounded border border-white/10">
				...
				<div className="h-48">
					<Suspense fallback={<div className="text-xs text-white/50">Loading chart...</div>}>
						<LazyPieBundle data={chartData} palette={palette} />
					</Suspense>
				</div>
